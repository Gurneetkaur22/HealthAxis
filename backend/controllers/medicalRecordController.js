const { MedicalRecord, Doctor, Patient, Appointment, User } = require('../models/index');
...
exports.getMedicalRecords = async (req, res, next) => {
  try {
    ...
    const records = await MedicalRecord.findAll({ where, include: [patientInclude, doctorInclude, apptInclude], order: [['date', 'DESC']] });
...
exports.getMedicalRecord = async (req, res, next) => {
  try {
    const record = await MedicalRecord.findByPk(req.params.id, { include: [patientInclude, doctorInclude, apptInclude] });
...
exports.createMedicalRecord = async (req, res, next) => {
  try {
    const { patient, appointment, diagnosis, treatmentNotes, followUpDate } = req.body;
    const doctor = await Doctor.findOne({ where: { userId: req.user.id } });
    if (!doctor) return res.status(400).json({ message: 'Doctor profile not found' });

    const record = await MedicalRecord.create({ patientId: patient, doctorId: doctor.id, appointmentId: appointment, diagnosis, treatmentNotes, followUpDate });
    const populated = await MedicalRecord.findByPk(record.id, { include: [patientInclude, doctorInclude] });
